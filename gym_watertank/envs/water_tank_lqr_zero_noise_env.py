...
class WaterTankLQRZeroNoiseEnv(gym.Env):
	...
	def __init__(self, A=np.array([[0.98, 0], [0.02, 0.98]]),
		...
		'''
		super(WaterTankLQRZeroNoiseEnv, self).__init__()
		self.m = A.shape[0]
	...
	def step(self, action_tilde):
		...
		if self.n==1:
			...
			new_state = self.state@self.A.T + action@self.B.T
...
class WaterTankLQRZeroNoiseEnv1(WaterTankLQRZeroNoiseEnv):
	metadata = {'render.modes': ['human']}
	def __init__(self, A=np.array([[0.98, 0], [0.02, 0.98]]),
					   B=np.array([[0.1],[0]]),
					   r = 9.,
		...
		Z1 is the because the the second tank is the only goal
			for the first tank, the cost is whether overflow
		'''
		super(WaterTankLQRZeroNoiseEnv1, self).__init__()
